...
func (s *service) processCommand() {
	...
	if err != nil {
		...
		s.healthcheck.msg = fmt.Sprintf("destination: %s unhealthy. Error: %s", h.dest, err)
		return
	}

	s.state = Healthy
	s.updateAtStateSince()
	s.healthcheck.msg = fmt.Sprintf("destination: %s OK (healthy)", h.dest)
}

// Needs to be converted to PER SERVICE proceessing, also paralel such that results trickle back to main independently
func processHeathchecks(services []service) <-chan service {
